...
class DFN(object):
    ...
    #embedding 
        ...
        self.pos_w_clicked = tf.Variable(init_seq(shape=[self._clicked_item_dim + self._pos_item_dim, self._item_dim]), dtype=tf.float32, name="pos_w_clicked")
        self.pos_w_unclick = tf.Variable(init_seq(shape=[self._unclick_item_dim + self._pos_item_dim, self._item_dim]), dtype=tf.float32, name="pos_w_unclick")
        self.pos_w_feedback = tf.Variable(init_seq(shape=[self._feedback_item_dim + self._pos_item_dim, self._item_dim]), dtype=tf.float32, name="pos_w_feedback")
        ...
        for i in range(0, self._hist_size):
            ...
            # 特征和位置embedding
            # batch_size, len(clicked_group_ids) * embed_dim
            clicked_z = tf.matmul(clicked_pos, self.pos_w_clicked)
            unclick_z = tf.matmul(unclick_pos, self.pos_w_unclick)
            feedback_z = tf.matmul(feedback_pos, self.pos_w_feedback)
        ...
        self.embed_wide = tf.Variable(tf.zeros([self._feature_size, 1]), name="embedding_wide")
        for group_id in main_group_ids:
            # batch_size, len(main_group_ids) * 1
            embedding_wide = tf.nn.embedding_lookup_sparse(self.embed_wide, self._group_feature["main_" + str(group_id)], sp_weights=None, combiner='mean')
        ...
        for group_id in candidate_group_ids:
           ...
           embedding_wide = tf.nn.embedding_lookup_sparse(self.embed_wide, self._group_feature["candidate_" + str(group_id)], sp_weights=None, combiner='mean')
    ...
    def attention(self, candidate_embedding, hist_embeddings, hist_embedding_dim, hisLens, prefix=""):
        ...
        self.attW1 = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[hist_embedding_dim * 4, attention_hidden_]), name=prefix + "attention_hidden_w1", dtype=tf.float32)
        self.attB1 = tf.Variable(tf.zeros(shape=[attention_hidden_]), name=prefix + "attention_hidden_b1", dtype=tf.float32)

        self.attW2 = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[attention_hidden_, 1]), name=prefix + "attention_hidden_w2", dtype=tf.float32)
        self.attB2 = tf.Variable(tf.zeros(shape=[1]), name=prefix + "attention_hidden_b2", dtype=tf.float32)
    ...
    def transformer(self, candidate_embedding, hist_embeddings, hist_embedding_dim, hisLens, prefix=""):
        ...
        for i in range(0, headnum):
            self.attQ_w = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[hist_embedding_dim, int(hist_embedding_dim / headnum)]), name=prefix + "attQ_w" + str(i), dtype=tf.float32)
            self.attK_w = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[hist_embedding_dim, int(hist_embedding_dim / headnum)]), name=prefix + "attK_w" + str(i), dtype=tf.float32)
            self.attV_w = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[hist_embedding_dim, int(hist_embedding_dim / headnum)]), name=prefix + "attV_w" + str(i), dtype=tf.float32)
        ...
        #FFN
        self.FFN_w0 = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[hist_embedding_dim, hist_embedding_dim * 4]), name=prefix+'FFN_w0', dtype=tf.float32)
        self.FFN_b0 = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[hist_embedding_dim * 4]), name=prefix + 'FFN_b0', dtype=tf.float32)
        
        self.FFN_w1 = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[hist_embedding_dim * 4, hist_embedding_dim]), name=prefix+'FFN_w1', dtype=tf.float32)
        self.FFN_b1 = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[hist_embedding_dim]), name=prefix+'FFN_b1', dtype=tf.float32)
    ...
    def stacked_fully_connect(self, x, dims, activation='relu', prefix='deep'):
        ...
        for i in range(0, dim_size):
            self.w = tf.Variable(tf.keras.initializers.GlorotNormal()(shape=[dims[i], dims[i + 1]]), name=prefix + 'w' + str(i), dtype=tf.float32)
            self.b = tf.Variable(tf.zeros(shape=[dims[i + 1], ]), name=prefix + 'b' + str(i), dtype=tf.float32)
            hidden = tf.matmul(hidden, self.w) + self.b
            if prefix != 'output':
